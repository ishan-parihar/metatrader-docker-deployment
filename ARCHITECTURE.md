...
| v16 (2026-09-06) | **v16-accelerated-cent live**: 9 streams (magics 992101–109, BTCUSDc/GBPJPYc/XAGUSDc/XAUUSDc), **300-tree** model, BookGovernor **trail@40%** + graduation freeze @100k USC. Banner format changed: **no `box=` field, new `atomCode=N`**. Bot reads all strategy defs from `stream_defs.py` (generated from bundle `*.set`); metals-8/101-tree map kept frozen as `LEGACY_*`. **Era rule**: magics were recycled on every redeploy, so per-trade attribution uses close time (`era_of`/`era_label`) across four eras — P1 141-tree (pre-07-30, trough 6,253.80 @ 07-24), P2 231-tree (07-30–08-31), P3 101-tree (leftover closes only), v16 (no trades yet). The 4 Sep-7 magic-0 closes were opened under P3 (ENTRY Sep 2-3) and bulk-closed manually — pinned to P3 via `LEFTOVER_CLOSES`, never v16. Dashboard shows per-fold aggregates (reconcile with EA inception totals); rolling Month/Week carry a swap-span note only while legacy trades sit in-window. Hourly equity curve skips the Fri-21:00–Sun-22:00 UTC halt (`_market_closed`). |
